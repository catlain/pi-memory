...
export default function memoryToolsExtension(pi: ExtensionAPI) {
	...
	pi.registerTool({
		...
		parameters: Type.Object({
			scope: Type.Optional(
				Type.Union(
					[Type.Literal("all"), Type.Literal("L1"), Type.Literal("L2")],
					{
						description: "扫描范围：all=全部，L1=全局，L2=项目。默认 all。",
						default: "all",
					},
				),
			),
		...
		async execute(
			...
			if (entries.length === 0) {
				return {
					content: [{ type: "text", text: "未找到任何记忆文件。" }],
					details: {},
				};
			}
			...
			for (const e of entries) {
				...
				lines.push(
					`| ${topic} | ${kws} | ${e.lines}${lineWarning} | ${e.description} |`,
				);
			}
	...
	pi.registerTool({
		...
		description: fs
			.readFileSync(
				path.join(__dirname, "prompts", "update-description.md"),
				"utf-8",
			)
			.trim(),
		parameters: Type.Object({
			fileName: Type.String({
				description: "目标文件名，格式 topic--kw1,kw2,kw3.md",
			}),
			content: Type.String({ description: "完整的记忆文件内容（markdown）" }),
			scope: Type.Optional(
				Type.Union([Type.Literal("L1"), Type.Literal("L2")], {
					description: "目标作用域。默认 L2。",
					default: "L2",
				}),
			),
		}),
		async execute(
			_toolCallId,
			params: { fileName: string; content: string; scope?: "L1" | "L2" },
			_signal,
			_onUpdate,
			ctx,
		) {
			...
			if (fileName.includes("/") || fileName.includes("\\")) {
				return {
					content: [{ type: "text", text: "❌ 文件名不允许包含路径分隔符" }],
					details: {},
				};
			}
			...
			if (contentBytes > MAX_CONTENT_BYTES) {
				return {
					content: [
						{
							type: "text",
							text: `❌ 内容 ${(contentBytes / 1024).toFixed(1)}KB 超过 ${MAX_CONTENT_BYTES / 1000}KB 限制`,
						},
					],
					details: {},
				};
			...
			if (contentLines > MAX_FILE_LINES) {
				return {
					content: [
						{
							type: "text",
							text: `❌ 内容 ${contentLines} 行，超过 ${MAX_FILE_LINES} 行硬限制，请精简或拆分后重试`,
						},
					],
					details: {},
				};
			...
			if (effectiveTotal > HARD_FILE_LIMIT) {
				return {
					content: [
						{
							type: "text",
							text: `❌ ${effectiveScope} 记忆文件已达 ${totalFiles} 个（硬限制 ${HARD_FILE_LIMIT}），拒绝写入。请先用 memory_index 查看，合并/删除后重试。`,
						},
					],
					details: {},
				};
			...
			return {
				content: [
					{
						type: "text",
						text: `✅ ${action}: ${fileName} (${contentLines} 行)${totalWarning}`,
					},
				],
				details: {},
			};
