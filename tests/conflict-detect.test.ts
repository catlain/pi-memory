...
describe("detectConflicts", () => {
	it("空目录无冲突", () => {
		const result = detectConflicts(TMP_DIR, "new--a,b.md", {
			topic: "new",
			keywords: ["a", "b"],
		});
	...
	it("同 topic 检测为冲突", () => {
		...
		const result = detectConflicts(TMP_DIR, "coding--new.md", {
			topic: "coding",
			keywords: ["new"],
		});
	...
	it("关键词重叠 ≥ 3 检测为冲突", () => {
		fs.writeFileSync(
			path.join(TMP_DIR, "existing--a,b,c,d.md"),
			"# Existing\n",
			"utf-8",
		);
		const result = detectConflicts(TMP_DIR, "new--a,b,c.md", {
			topic: "new",
			keywords: ["a", "b", "c"],
		});
		expect(result).toHaveLength(1);
	...
	it("关键词重叠 < 3 不检测为冲突", () => {
		fs.writeFileSync(
			path.join(TMP_DIR, "existing--a,b,c.md"),
			"# Existing\n",
			"utf-8",
		);
		const result = detectConflicts(TMP_DIR, "new--a,b.md", {
	...
	it("跳过 MEMORY.md", () => {
		...
		const result = detectConflicts(TMP_DIR, "new--a.md", {
			topic: "new",
			keywords: ["a"],
		});
		expect(result).toEqual([]);
	...
	it("跳过自身文件名", () => {
		...
		const result = detectConflicts(TMP_DIR, "me--a,b,c.md", {
			topic: "me",
			keywords: ["a", "b", "c"],
		});
		expect(result).toEqual([]);
	...
	it("多个冲突同时检测", () => {
		...
		fs.writeFileSync(
			path.join(TMP_DIR, "other--a,b,c.md"),
			"# B\nl1\n",
			"utf-8",
		);
		const result = detectConflicts(TMP_DIR, "coding--a,b,c.md", {
			topic: "coding",
			keywords: ["a", "b", "c"],
		});
